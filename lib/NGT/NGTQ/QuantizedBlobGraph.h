...
#endif
      if (objectType != NGT::ObjectSpace::ObjectTypeNone) {
        NGT::ObjectSpace::quantizeToQint8(fq, dimension, cqueries + dimension * qi, objectType, offset,
                                          scale);
      }
    }
    ...
    for (size_t bi = 0; bi < blobList.size(); bi++) {
      ...
      for (size_t qi = 0; qi < queryList.size(); qi++) {
        ...
        for (size_t di = 0; di < noOfObjects; di++) {
          ...
          searchContainer.batchResult[queryList[qi]].emplace_back(
              NGT::ObjectDistance(quantizedBlobGraph[blobID].ids[di], distances[bi][qi * noOfObjects + di]));
        }
      }
    }
#pragma omp parallel for
    for (size_t i = 0; i < searchContainer.batchResult.size(); i++) {
      std::sort(searchContainer.batchResult[i].begin(), searchContainer.batchResult[i].end());
    ...
    std::vector<std::priority_queue<NGT::ObjectDistance, std::vector<NGT::ObjectDistance>,
                                    std::less<NGT::ObjectDistance>>>
        resultSet(searchContainer.numOfQueries);
    for (size_t bi = 0; bi < blobList.size(); bi++) {
      auto blobID = blobList[bi].first;
      //auto subspaceID = quantizedBlobGraph[blobID].subspaceID;
      std::vector<uint32_t> &queryList = blobList[bi].second;
...
#pragma omp parallel for
  ...
  std::vector<std::priority_queue<NGT::ObjectDistance, std::vector<NGT::ObjectDistance>,
                                  std::less<NGT::ObjectDistance>>>
      resultSet(searchContainer.numOfQueries * nOfThreads);
#pragma omp parallel for
  for (size_t bi = 0; bi < blobList.size(); bi++) {
...
#pragma omp parallel for
  for (size_t qi = 0; qi < searchContainer.numOfQueries; qi++) {
    ...
    std::sort(searchContainer.batchResult[qi].begin(), searchContainer.batchResult[qi].end());
